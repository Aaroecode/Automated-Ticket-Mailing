...
async def index():
    ...
    if status is True :
        ...
        for member in content["membersData"]["membersData"]:
            ...
            Config.logger.log(20, f"User Registered with Transaction id: {transactionId}, Event Name: {event}, Team Name: {TeamName}, Team ID: {TeamId}, Member ID: {id}, Name: {name}, Course: {course}, College: {contact}, Email: {email}")
            ...
            Config.sheet.append([date, TeamId, TeamName, event, id, name, course, contact, email, hashId])
            await Config.mail.send("Ticket Confirmation", "tickets@techrise.club",email, "Ticket Confirmation", template=os.path.join(os.getcwd(), "app", "assets", template),
                             ...
                             placeholder={"{Name of Attendee}":name, "{date}":eventDate, "{time}": eventTime, "{locations}": eventLocation, "{event}": event, "{ruleLink}": rules, "{teamId}": TeamId})
