...
class Mail():
    ...
    async def send(self, format_name: str, sender:str, reciever:str, subject:str, template, contents: Union[dict,None], placeholder: dict):
        ...
        while retryTime<7:
            try:
                context = ssl.create_default_context()
                with smtplib.SMTP_SSL(host=self.host, port=self.port, context=context) as mailer:
                    mailer.login(user=self.username, password=self.password)
                    mailer.sendmail(sender, reciever, messge.as_string())
